...
function generateBDS(tender: any) {
  ...
  // Хуулийн дагуу: бараанд 50-70%, ажилд 80-100%
...
function generateResults(tender: any) {
  ...
  // Realistic historical outcome simulation for concluded tenders
  const winnerPrice = Math.round(budget * (0.88 + Math.random() * 0.08));
  const savings = budget - winnerPrice;
  const savingsPct = Math.round((savings / budget) * 100);

  const competitor2Price = Math.round(budget * 0.98);
  const competitor3Price = Math.round(budget * 1.02);
